...
const EXISTING_BUSINESS_UPDATES: Record<string, { routeDay: number; isBonus: boolean; buildingGroup?: string | null }> = {
  "Brooklyn Center Roofing": { routeDay: 1, isBonus: false },
  "Columbia Heights Siding": { routeDay: 1, isBonus: false },
  "Northeast Minneapolis Gutters": { routeDay: 1, isBonus: false },
  "Nokomis Home Services": { routeDay: 2, isBonus: false },
  "Richfield Roofing": { routeDay: 2, isBonus: false },
  "South Minneapolis Contractors": { routeDay: 2, isBonus: false },
  "Bloomington Roofing Pros": { routeDay: 3, isBonus: false },
  "I-494 Home Repair": { routeDay: 3, isBonus: false },
  "Lyndale Contractors": { routeDay: 3, isBonus: false },
  "St. Louis Park Home Services": { routeDay: 4, isBonus: false },
  "Spring Lake Park Roofing": { routeDay: 4, isBonus: false },
  "West Metro Contractors": { routeDay: 4, isBonus: false },
};
...
async function main() {
  ...
  for (const business of BUSINESSES) {
    if (existingNames.has(business.name.toLowerCase())) {
      await db
        ...
        .where(eq(businessesTable.name, business.name));
  ...
  for (const [name, updates] of Object.entries(EXISTING_BUSINESS_UPDATES)) {
    const match = existing.find((b) => b.name.toLowerCase() === name.toLowerCase());
    if (match) {
      await db
        .update(businessesTable)
        .set({ routeDay: updates.routeDay, isBonus: updates.isBonus, buildingGroup: updates.buildingGroup ?? undefined })
        .where(eq(businessesTable.id, match.id));
      console.log(`  Route-tagged existing: ${name}`);
    }
  }
  ...
  console.log(`\nTotal businesses: ${all.length} (inserted: ${inserted}, updated/skipped: ${skipped})`);
  console.log("Done!");
  process.exit(0);
}
